  ...
  return (
    <div className="description">
      ...
      <div className="image-decription-card">
        {data.sponsorship?.tagline?<h1>{data.sponsorship?.tagline}</h1>:null}
        {data.alt_description?<p>DESCRIPTION :&nbsp;{data.alt_description}</p>:null}
        {data.user?.name ? <p>SPONSOR USER NAME: {data.user?.name} </p> : null}
        <p>TOTAL LIKES :{(liked && data.likes+1)|| data.likes}</p>
        {data.sponsorship?.sponsor?.bio?<p>BIO: {data.sponsorship?.sponsor?.bio}</p>:null}
      </div>
    </div>
  );
};
